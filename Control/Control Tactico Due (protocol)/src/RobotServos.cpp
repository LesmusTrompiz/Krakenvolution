...
RobotServo::RobotServo(const uint8_t _ID_AnalogRead, const uint8_t pwm_id)
: ID_AnalogRead(_ID_AnalogRead), PWM_ID(pwm_id)
{
  // Lectura analógica
  pinMode(ID_AnalogRead, INPUT);
}

RobotServo::RobotServo(const uint8_t pwm_id)
: ID_AnalogRead(-1), PWM_ID(pwm_id)
{}
...
void RobotServo::set_angle(float angle)
{
  ...
  servos.setPWM(PWM_ID, 0, (int)(nominal_tick_value));
}
