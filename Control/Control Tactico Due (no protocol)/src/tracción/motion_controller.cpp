...
void Odom::check_mov()
{
 ...
 // Parada absoluta
 parado_absoluto = cuentas_derecha <= 5 || cuentas_izquierda <= 5;
 // Parada estimada
...
void motion_controller::prev_move_calculus(bool movimiento)
{
  ...
  if(movimiento)
  {
    ...
    cal_trapecio.calculo_recta(ref_distancia, param_mecanicos.vel_max/2);
  ...
  else 
  {
    ...
    cal_trapecio.calculo_giro(ref_ang, param_mecanicos.vel_giro/2);
    motores.encender_motores();
    giro_en_curso = 1;
    recta_en_curso = 0;
    odom.parado = 0;
...
void motion_controller::move_control()
{
  ...
  if(recta_en_curso)
  {
    if(fabs(odom.pose_actual.x) < fabs(cal_trapecio.distancia_init_frenada - cal_trapecio.ajuste_distancia_recto)*(param_mecanicos.diam_rueda/2))
    {
      ...
      // Serial.println("Vc");
      motores.rmotor_vel = param_mecanicos.vel_max/2;
      motores.lmotor_vel = param_mecanicos.vel_max/2;
      motores.set_vel_rmotor();
      motores.set_vel_lmotor();
    }
    else if(fabs(odom.pose_actual.x) < fabs(cal_trapecio.distancia_total_rad)*(param_mecanicos.diam_rueda/2)-ajuste_error_tactico)
    {
      // Serial.println("F");
      // Velocidad de freno
  ...
  else if(giro_en_curso)
  {
    if(fabs(DEG2RAD(odom.pose_actual.alfa)*param_mecanicos.L_eje/(param_mecanicos.diam_rueda)) < fabs(cal_trapecio.distancia_init_frenada - cal_trapecio.ajuste_distancia_giro))
    {
      // Velocidad de crucero
      // Actualizamos la velocidad
      motores.rmotor_vel = param_mecanicos.vel_giro/2;
      motores.lmotor_vel = param_mecanicos.vel_giro/2;
      motores.set_vel_rmotor();
      motores.set_vel_lmotor();
    }
    else if(fabs(DEG2RAD(odom.pose_actual.alfa)*param_mecanicos.L_eje/(param_mecanicos.diam_rueda)) < fabs(cal_trapecio.distancia_total_rad - ajuste_error_tactico))
    {
      // Velocidad de freno
      motores.rmotor_vel = param_mecanicos.vel_freno;
      motores.lmotor_vel = param_mecanicos.vel_freno;
